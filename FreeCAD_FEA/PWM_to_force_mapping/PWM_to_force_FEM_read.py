...
# Finding points just on the surface of the mirror and their displacement (i.e where z_point=0)
    ...
    Z_2D=read_xyz.hard_aperture(Z_2d,250)
...
def PWM_to_force(FEM,interferogram_files,PWM_values,force_values,fig_number):#has to be an array of all equal lengths
    ...
    z_2d_max=np.max(z_2d[len(z_2d)-1])
    z_2d_index=np.where(z_2d[len(z_2d)-1] == z_2d_max)
    #print(z_2d[1][z_2d_index])
    # print(z_2d_index)
    for i in range(len(z_2d)):
        displacements_inter.append(z_2d[i][z_2d_index])
        #print(np.min(z_2d[i][z_2d[i]>0]))
        # z_min=np.min(z_2d[i][z_2d[i]>0])
        # z_max=np.max(z_2d[i])
        # #print(z_max)_min)
        # displacements_inter.append(z_max-z
