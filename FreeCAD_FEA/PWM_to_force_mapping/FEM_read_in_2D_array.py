...
def zernike_decomposition(z,fig_number,zernike_term_file_title,x_zernike=None,y_zernike=None):
    popt=[]
    z_data=z#
    ...
    for i in range(len(Zernike_func)):
        ...
        popt.append(popt_i)
        print('zernike term:')
        print(Zernike_func[i])
        print(popt_i)
    y=pickle.dumps(popt)
    np.save(zernike_term_file_title,y)
        
        
        
    z_guess=np.zeros((x_zernike,y_zernike))
    ...
    for i in range(len(Zernike_func)):
        ...
        x_z=np.linspace(-1,1,x_z_map)
        y_z=np.linspace(-1,1,y_z_map)
        z_guess+=Zernike_func[i](x_z,y_z,popt[i][0],popt[i][1],popt[i][2])
