...
def read(filename,cornersize):
    ...
    pixel_background=np.zeros((y_max,x_max))
   
    pixels_coord=np.mgrid[0:y_max,0:x_max].reshape(2,-1).T
    
   
    
    pixel_background[0:y_max,0:x_max]=(pixels_coord[0:len(pixels_coord),0]*grads[0]+pixels_coord[0:len(pixels_coord),1]*grads[1])+corners[0] 
