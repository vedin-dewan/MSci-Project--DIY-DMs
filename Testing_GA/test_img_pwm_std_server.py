...
"""
"""
This takes data to measure the noise in the camera images while
switching off and on the mirror to the same PWM value (gives noise in mirror values)
THIS GOES ON THE RASPBERRY Pi
"""
...
def read_in_img(filename,k):
    ...
    while total_pix_vals<500000 or max_pix_val>=255:
        time.sleep(1.0)
        take_pic='fswebcam -r 1280x720 --no-banner /home/pi/Test_Image_std/focal_%i.jpg'%k
        os.system(take_pic)
        pixel_values= io.imread('/home/pi/Test_Image_std/focal_%i.jpg'%k, as_gray=True)
        pixel_values=pixel_values*255
        total_pix_vals=np.sum(pixel_values)
        max_pix_val=np.max(pixel_values)
...
def read(filename,cornersize,k):
    
    
    pixels=read_in_img(filename,k)
    y_max=len(pixels)#rows
    x_max=len(pixels[0])#columns
    '''
...
def main():
    ...
    fitness_params=np.zeros((5,20,10))
    for i in range(5):
        ...
        for j in range(20):
            ...
            for k in range(10):
                ...
                take_pic='fswebcam -r 1280x720 --no-banner /home/pi/Test_Image_std/focal_%i.jpg'%k
                os.system(take_pic)
                fitness_params[i][j][k]=read('/home/pi/Test_Image_std/focal_%i.jpg'%k,19,k)
