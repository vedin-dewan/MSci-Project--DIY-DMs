...
# def mutate(parents,offsprings):
#     fit_params=np.zeros(no_parents)
#     for j in range(no_parents):
#         fit_params[j]=fitness_func(parents[j])
      
#     for i in range(no_parents):
#         mut_gene_ind=np.random.randint(0,no_genes)
#         print(offsprings[i])
#         mut_gene_value_dec=np.random.normal(offsprings[i],255)
...
def main():
    ...
    for generation in range(max_it):
        #select parents
        start=time.time()
        parents,command=selection(new_pop)
